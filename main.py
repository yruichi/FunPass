...
class AdminDashboard:
    ...
    def __init__(self, root):
        ...
        self.root.bind('<<PriceUpdate>>', lambda e: self.load_employees())
        self.show_dashboard()
    ...
    def show_dashboard(self):
        ...
        # Place all dashboard widgets inside dashboard_frame 
        ...
        for idx, (label, value, color) in enumerate(stats_data):
            ...
            tk.Label(stat, text=value, font=('Segoe UI', 22, 'bold'), fg=color, bg='#E5ECCB').pack(anchor='w', padx=10, pady=(0, 8))
    ...
    def show_rides(self):
        ...
        # Title
        title = tk.Label(header_row, text="Pass Types and Inclusions", font=('Segoe UI', 22, 'bold'), bg='#F0E7D9', fg='black', anchor='w')
        title.pack(anchor='w')
        # Subtitle
        subtitle = tk.Label(header_row, text="View rides descriptions and inclusions", font=('Segoe UI', 15), fg='#6b7280', bg='#F0E7D9', anchor='w')
    ...
    def show_employee_management(self):
        ...
        add_btn = ctk.CTkButton(
            controls_bar, text="+ Add Account", width=140, height=36, fg_color="#9A4E62", text_color="#fff", hover_color="#AB7986",
    ...
    #        self.customers_tree.column(col, width=120)
        for col in columns:
            if col == "Amount":
                ...
                self.customers_tree.heading(col, text=col)
                self.customers_tree.column(col, width=120, anchor='w')  
            elif col == "Ticket ID":
                self.customers_tree.heading(col, text=col)
                self.customers_tree.column(col, width=120, anchor='center') 
            elif col == "Employee":
                self.customers_tree.heading(col, text=col)
                self.customers_tree.column(col, width=120, anchor='w')  # left-align Employee column
            else:
                self.customers_tree.heading(col, text=col)
                self.customers_tree.column(col, width=120, anchor='center')  
    ...
    def show_cancellations(self):
        ...
        cancel_subtitle = ctk.CTkLabel(header_row, text="View and Manage Customers Submitted Refund Requests", font=("Segoe UI", 15, 'normal'), text_color="#6b7280")
        ...
        # Delete Button (if not already present)
        delete_btn = ctk.CTkButton(
            controls_bar,
            text="Delete",
            width=90,
     ...
     #       bg='#f44336', fg='white', font=('Arial', 11)).pack(side=tk.LEFT, padx=5)
        ...
        for col in columns:
            ...
            if col in ["Name", "Reason", "Email", "Pass Type", "Status"]:
                self.cancellations_tree.column(col, width=width, anchor='w')
            else:
                self.cancellations_tree.column(col, width=width, anchor='center')
        def clear_selection_on_click(event):
    ...
    def show_pricing(self):
        ...
        pricing_title = tk.Label(self.content_frame, text="Pass Type Pricing", font=('Segoe UI', 22, 'bold'), bg='white', anchor='w')
   ...
   #     self.price_update_label = tk.Label(self.content_frame, text="", font=('Arial', 10), fg='#4CAF50', bg='white', anchor='w')
    #    self.price_update_label.pack(pady=(5, 0), padx=20, anchor='w')
        ...
        pricing_subtitle = tk.Label(self.content_frame, text="View and Manage Ticketing Pricing", font=('Segoe UI', 15), fg='#6b7280', bg='white', anchor='w')
    ...
    def show_employee_dialog(self, mode="add", event=None):
        ...
        for label_text, field_name in alloc_fields:
            ...
            # For ticket quantity with default value 0 and hint behavior
